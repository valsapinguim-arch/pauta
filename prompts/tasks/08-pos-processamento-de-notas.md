...
- Justificação: 60 ms é mais curto do que qualquer nota musicalmente intencional em andamentos plausíveis (uma semicorchea a 200 BPM tem 75 ms), mas mais longo do que os *cliques* e transientes que o modelo interpreta como notas. É intencionalmente um pouco mais permissivo do que o `MIN_NOTE_LENGTH_MS` do modelo (Tarefa 7), para que o corte definitivo aconteça aqui, num sítio testável, e não dentro do modelo.
