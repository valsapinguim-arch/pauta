...
Depende da Tarefa 12 (`ScoreDocument`) e da Tarefa 3 (*slot* da pauta na `ResultView`). É o momento em que o produto passa a ser verificável: até aqui a qualidade da transcrição era invisível.
...
- `ResizeObserver` no contentor, com *debounce*.
...
* Implementar `@/features/notation/ScoreView`:
  ...
  * `ResizeObserver` com *debounce* (decisão 5)
...
* Preencher o *slot* da pauta na `ResultView` (Tarefa 3)
...
* "Redimensionamento é observado com `ResizeObserver` e sempre com *debounce*."
