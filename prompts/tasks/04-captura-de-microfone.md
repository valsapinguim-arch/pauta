...
- **O `AudioWorklet` precisou de um TERCEIRO tsconfig.** `src/workers/recorder.worklet.ts` corre em `AudioWorkletGlobalScope`, que não é DOM nem `WebWorker` — tem o seu próprio `self`, o seu próprio `Event`/`MessagePort` (redeclarados de raiz por `@types/audioworklet`, incompatíveis com os de `lib.dom`). Criado `tsconfig.worklet.json` (`types: ["audioworklet"]`, `lib: ["ES2022"]` só), excluído tanto do `tsconfig.json` principal como do `tsconfig.worker.json` da Tarefa 2. `pnpm typecheck` agora corre três tsconfigs.
- **`new URL('./x.worklet.ts', import.meta.url)` NÃO chega para o Vite bundlar um AudioWorklet.** Ao contrário de `new Worker(new URL(...))` (que o Vite reconhece e trata especialmente), `audioContext.audioWorklet.addModule(url)` é só uma chamada de método genérica — o Vite não a intercepta, e o `new URL()` sozinho limitava-se a apontar para o `.ts` cru, nunca transpilado. O build passava sem avisos e a falha só apareceria em runtime, ao tentar carregar TypeScript como JavaScript no browser — o pior tipo de falha, silenciosa até ao fim. Corrigido com o sufixo documentado do próprio Vite: `import url from '@/workers/recorder.worklet.ts?worker&url'`. Confirmado a olhar para `dist/assets/` depois de `pnpm build`: tem de aparecer um `recorder.worklet-<hash>.js` à parte, com o `calculateRms` de `@/lib/audio/rms.ts` já embutido lá dentro.
- **`calculateRms` ficou em `@/lib/audio/rms.ts`, não dentro do worklet.** É matemática pura sobre `Float32Array` — corre sem alterações tanto no `AudioWorkletGlobalScope` (sem `lib.dom`) como em Node (testável em segundos, sem áudio real nenhum). Estabelece a pasta `@/lib/audio/` que a Tarefa 6 vai encher com o resto (mono, reamostragem, corte de silêncio, normalização).
- **A versão instalada do `eslint-plugin-react-hooks` (7.x) rejeita mutar `ref.current` durante o render.** O padrão comum "guardar as props mais recentes numa ref para um callback assíncrono usar depois" (`optionsRef.current = options` direto no corpo do hook) passou a dar erro de lint (`react-hooks/refs`). Resolvido movendo a atribuição para dentro de um `useEffect([options])` — um render de atraso antes da ref refletir o valor novo, irrelevante aqui porque só é lida de dentro de callbacks assíncronos (nunca durante outro render).
- **Verificação end-to-end real, apesar de o painel de browser bloquear acesso a microfone.** O painel avisa explicitamente que pedidos de `getUserMedia` são bloqueados nesta sessão — mas isso serviu, sem querer, para verificar o caminho de erro por inteiro: cliquei em "Gravar" → apareceu a explicação de permissão (só da primeira vez; a segunda vez que testei, saltou-a, confirmando que `localStorage` funciona) → "Continuar" → `getUserMedia` rejeitou → `mapGetUserMediaError` mapeou para `permission-denied` → `ErrorView` mostrou exatamente a mensagem escrita para esse código ("Sem acesso ao microfone… Autoriza o acesso ao microfone nas definições do browser…") → "Tentar novamente" voltou a `idle`. O que não foi possível verificar nesta sessão: a captura em si (RMS real, corte aos 60 s, deteção de `too-quiet`) — fica para verificação manual num Chrome/Edge/Safari a sério, com microfone físico, tal como já assinalado na Tarefa 2 para o service worker.
