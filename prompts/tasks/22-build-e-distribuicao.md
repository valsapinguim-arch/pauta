...
**8. Um só ambiente, sem _staging_**

- Publicação direta para produção a partir de `main`, com pré-visualizações automáticas por PR se o serviço as oferecer.
- Justificação: a app não tem base de dados nem estado de servidor; um ambiente de _staging_ separado duplicaria configuração para testar o que uma pré-visualização de PR já testa. As pré-visualizações cobrem o risco real (uma alteração quebrar a app) sem infraestrutura adicional.
...
- Confirmar que os ficheiros WASM do TensorFlow.js são servidos com o `Content-Type` correto (`application/wasm`) — servidos como `application/octet-stream`, a instanciação em _streaming_ falha e o TensorFlow.js recorre a um caminho mais lento, silenciosamente.
