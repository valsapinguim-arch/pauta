...
Depende da Tarefa 1 (tokens, máquina de estados, `@/strings`). É a tarefa que define o que o utilizador vê; as Tarefas 4 a 15 ligam funcionalidade real aos _slots_ deixados aqui.
...
- Botão de texto abaixo do botão principal, mais zona de _drop_ em ecrãs largos.
- Justificação: em telefone, drag & drop não existe; em desktop, é o gesto esperado. Um botão sempre visível funciona nos dois, e a zona de _drop_ acrescenta-se só onde faz sentido.
...
- Apenas _feedback_ imediato (pressão de botão, indicador de nível, progresso).
...
- Deixar _slots_ explícitos, com `// TODO Tarefa N:` a identificar quem os preenche:
  - `onStartRecording` → Tarefa 4
  - `onPickFile` / zona de _drop_ → Tarefa 5
...
- Depende da Tarefa 1.
- Os _slots_ desta tarefa são o contrato com as tarefas seguintes. Se uma tarefa posterior precisar de mudar a forma de um _slot_, atualizar aqui o componente — nunca criar uma view alternativa em paralelo.
- A `ResultView` mostra nesta fase uma imagem estática de pauta como _placeholder_. Não tentar desenhar notação à mão em SVG para fazer de conta — VexFlow é da Tarefa 13 e uma implementação intermédia seria descartada.
...
- **`exactOptionalPropertyTypes` (Tarefa 0) exige `| undefined` explícito em props opcionais que recebem `condição ? x : undefined`.** Apanhou `ProgressProps.value`/`className` e `ToastProps.description`/`action` — `App.tsx` passa exatamente esse padrão para os toasts de instalação/atualização. Documentado no `AGENTS.md` para não se repetir a mesma investigação a cada componente novo com uma prop condicional.
- **`@testing-library/react` não limpa o DOM sozinho.** Sem `test.globals: true` (decisão deliberada — `describe`/`it`/`expect` importados explicitamente), a limpeza automática entre testes não deteta `afterEach` como global; sem isto, `render()` sucessivos no mesmo ficheiro acumulavam elementos e `getByRole` começava a encontrar duplicados. Resolvido com `afterEach(cleanup)` explícito em `src/test/setup.ts`.
- **jsdom não implementa `hasPointerCapture`.** O Radix Toast usa a API de Pointer Events para o gesto de arrastar (swipe-to-dismiss) e qualquer teste que dispare um evento de ponteiro sobre um `Toast` rebentava com "target.hasPointerCapture is not a function" — não é um bug do componente, é uma lacuna conhecida do jsdom ([jsdom#3029](https://github.com/jsdom/jsdom/issues/3029)). Polyfill mínimo em `src/test/setup.ts`, guardado atrás de `typeof Element !== 'undefined'` porque o mesmo `setupFiles` corre também para os testes de `@/lib` em ambiente `node`, onde `Element` nem existe.
- **`isIosManualInstall` e `canInstall` (Tarefa 2) partilham um único `Toast` em `App.tsx`.** São mutuamente exclusivos na prática (um é Chromium, o outro especificamente iOS Safari) — juntar poupa um segundo toast quase idêntico. O texto muda conforme o caso: com `action` quando há `beforeinstallprompt` real, só com `description` (as instruções manuais) em iOS.
- **Limite de verificação visual neste ambiente.** O painel de browser embutido (Electron, já registado na Tarefa 2) não compõe frames para `computer.screenshot` nesta sessão — sem imagem, a verificação em 320/768/1280px e em claro/escuro foi feita por inspeção programática: `scrollWidth` vs. `innerWidth` (sem overflow horizontal em nenhuma largura), dimensões computadas dos alvos de toque (`getBoundingClientRect`, todos ≥44×44px), e os tokens CSS resolvidos (`getComputedStyle`, cores de tema claro/escuro corretas). Cobre o que os "Entregáveis" pedem, mas não substitui olhar para a pauta a sério — vale a pena confirmar visualmente num Chrome real antes de dar por fechado.
- **A ferramenta de preview (`preview_start`) resolve `.claude/launch.json` a partir do diretório de trabalho fixo da sessão (`cath-fit`), não do projeto atual.** Pedir `{name: "pauta"}` devolveu sem aviso a configuração `"web"` do `cath-fit` (mesma porta, 5173) — só se notou porque o conteúdo da página não batia certo. Contorno usado: arrancar `pnpm dev --port <outra porta> --strictPort` manualmente e abrir esse separador por `navigate` direto, sem passar pela resolução por nome. Relevante para quem for verificar as próximas tarefas visualmente nesta mesma sessão.
