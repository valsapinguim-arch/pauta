...
- **TypeScript fixado em `^6.0.3`, não na versão mais recente.** O `typescript-eslint` 8.x recusa-se a arrancar com a API do TypeScript 7 (`typescript-eslint does not support TS 7.0`), o que faz `pnpm lint` falhar por completo — não degradar, falhar. O TS 7 removeu também a opção `baseUrl`, pelo que os `paths` do `tsconfig.json` passaram a resolver-se relativamente ao próprio ficheiro (`"@/*": ["./src/*"]`), forma que funciona nas duas versões. Rever quando o `typescript-eslint` suportar TS 7 ([#10940](https://github.com/typescript-eslint/typescript-eslint/issues/10940)).
- **`subject-case` desativado no commitlint.** A regra por omissão do `config-conventional` rejeita maiúscula inicial no assunto, o que colide com descrições em pt-PT. `header-max-length` subiu para 100.
- **Formatação retroativa.** Os ficheiros de planeamento foram escritos antes de o Prettier existir no repositório, pelo que a sua adoção gerou um diff grande e puramente cosmético (marcadores de lista `*` → `-`, linha em branco após títulos a negrito, `*ênfase*` → `_ênfase_`). Sem alteração de conteúdo.
